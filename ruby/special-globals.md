...
- it is unique in every ruby file in your app e.g. _ consequences: _ `Dir.chdir`
  does nothing to this. it does not depend on the CWD of the process! _ the
  value in **FILE** depends on your shell CWD when you run the ruby process _ it
  is just the filename if the file is in the same dir as your shell CWD \* it is
  an absolute path if the file is in a different dir to your shell CWD
...
$: $LOAD*PATH * Array of dirs that ruby will search for filenames passed to
